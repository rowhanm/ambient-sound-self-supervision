...
#state_dict = torch.load("ambient-resnet-model_5.pt")
...
for epoch in range(EPOCHS):
    ...
    train_loss, train_acc = train(resnet, train_dl, optimizer, criterion, scheduler)
    valid_loss, valid_acc = evaluate(resnet, val_dl, criterion)
    print("[EPOCH " + str(epoch)+"]")
    if valid_loss < best_valid_loss:
        best_valid_loss = valid_loss
        torch.save(resnet.state_dict(), 'ambient-resnet-model_5.pt')
